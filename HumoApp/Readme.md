  ...
  - devolver JSON con al menos:
    {
      ...
      "category": "...",
      "score": 72,
      "signals": { "promesasLaborales": 30, "lenguajeExagerado": 20, ... },
      "explanation": "...",
      "createdAt": "2026-01-15T12:34:56Z",
      "modelVersion": "py-llm-v1",
      "raw": { /* opcional */ }
    }
- En errores: devolver formato uniforme:
...
1. Configura `.env` con `MONGO_CONNECTION_STRING`, `MONGO_DATABASE`, `PYTHON_API_URL`, (opcional) `PYTHON_API_KEY`.
2. Levanta MongoDB (local o Docker).
3. Levanta el servicio Python (FastAPI/uvicorn) y verifica `POST /api/analyze`.
4. Ejecuta la API .NET (Visual Studio 2026 o `dotnet run`).
5. Usa curl/Postman para probar POST/GET.
...
4. Implementar autenticación/usuarios y pruebas de seguridad antes de producción.
